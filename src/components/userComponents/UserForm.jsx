...
import { getSpecificUserData, updateUserData, postUserData } from "../../actions/Action";
...
const UserForm = ({updating}) => {
    ...
    const { id } = useParams();
    const { usersData, selectedUser, formLoader } = useSelector((state) => state.user);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            if (updating !== true) {
                dispatch(postUserData(formData)).unwrap().then(() => {
                    navigate("/home");
                })
            } else {
                dispatch(updateUserData({ formData, id })).unwrap().then(() => {
                    navigate("/home");
                })
            }
                ...
                >
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex gap-2 h-20 items-center ">
                            <label htmlFor="files" class="btn ml-2">Select Image(Optional)</label>
